...
更新已有关键条目时附其原 id，status 为 active（仍有效）、resolved（已解决）或 retracted（撤回），不变条目省略即可保留。旧猜测不得当事实，承诺兑现须有明确证据；未再次提及不等于完成。不要输出 sources、来源楼层列表或在条目正文后附来源标注；整理批次由插件自动关联，不需要模型填写。
